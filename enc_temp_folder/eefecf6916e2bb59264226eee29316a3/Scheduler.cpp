...
const int Scheduler::STARVATION = 10000;
...
void Scheduler::init() {
	...
	try {
		...
		// Create a thread for check starvation
		...
		// Create a thread for deadline task manager
		std::thread Deadline_Thread([this]() {
			SetThreadDescription(GetCurrentThread(), L"DeadlineHandler");
			spdlog::info(Logger::LoggerInfo::START_THREAD, "DeadlineHandler");
...
void Scheduler::execute(std::shared_ptr<Task> task) {
	...
	while (true) {
		...
		if (task->getRunningTime() == 0) {
			...
			totalRunningTask--;
			spdlog::info(Logger::LoggerInfo::TASK_COMPLETED, task->getId(), task->getPriority());
		...
		if (task->getPriority() != PrioritiesLevel::CRITICAL && !realTimeScheduler.getRealTimeQueue().empty()) {
			spdlog::info(Logger::LoggerInfo::TASK_PREEMPTIVE, task->getId(), task->getPriority());
			preemptive(task);
			return;
		}
	...
	while (true) {
		...
		if (!starvationCheckQueue.empty()) {
			if (starvationCheckQueue.front()->getStatus() != TaskStatus::CREATION) {
				...
				cout << "Starvation detected! Task ID: " << starvationCheckQueue.front()->getId();
				throw std::runtime_error("Starvation detected! Task ID: " + starvationCheckQueue.front()->getId());
			}
