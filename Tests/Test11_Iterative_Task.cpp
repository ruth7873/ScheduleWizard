...
//        CHECK(s.getIterativeTaskHandler().popIterativeTask() == nullptr);
//    }
//    clearAll(s);
...
//        std::this_thread::sleep_for(std::chrono::milliseconds(1)); // Wait 2 seconds for the fast task to be eligible for execution
//        auto poppedTask1 = s.getIterativeTaskHandler().popIterativeTask();
//        CHECK_EQ(poppedTask1->getExecutionInterval(), 1);
//
//        // Wait another few seconds for the slow task
//        std::this_thread::sleep_for(std::chrono::milliseconds(4)); // Wait an additional 4 seconds
//        auto poppedTask2 = s.getIterativeTaskHandler().popIterativeTask();
