////
//
//TEST_CASE("Test WeightRoundRobinScheduler") {
//    Scheduler scheduler;
...
TEST_CASE("Test WeightRoundRobinScheduler with Log File Verification") {
    ...
    SUBCASE("Run tasks and verify execution order in the log file") {
        ...
        std::cout << " number of tasks in higher queue: " << scheduler.getWrrQueuesScheduler().getWrrQueues()[PrioritiesLevel::HIGHER].queue.size();
        // Start the WeightRoundRobinScheduler thread
        std::thread WRRThread([&WRRscheduler]() {
        ...
        WRRThread.detach();  // Detach the thread to allow it to finish
